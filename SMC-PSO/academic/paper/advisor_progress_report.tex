...
\textbf{2.3\%} overshoot, and \textbf{74\%} chattering reduction (FFT metric).
All three quantified improvements are statistically significant
($p \le 0.038$; Cohen $d \ge 2.1$).
MT-8 disturbance logs are retained as qualitative stress-test evidence only,
because all controller-scenario entries are marked non-converged.

\textbf{Current completion priorities.}
(1)~Finalize STA gain-order consistency package for archival rigor
(see submission checklist in Appendix~\ref{app:issues}).
(2)~Keep Hybrid Adaptive outside MT-5 statistical ranking and retain it
for methodological completeness only.

\textbf{Thesis status.} 90/100 score.
January~2026 completion phases slipped due to ongoing research obligations.
Revised target: \textbf{April~2026} (see updated timeline in Section~\ref{sec:timeline_2026}).

\subsection*{Report-Level Summary Map (Flowchart)}
\begin{center}
\begin{tikzpicture}[
  font=\scriptsize,
  node distance=3.6mm,
  stage/.style={
    draw=headblue!80, rounded corners=2pt, align=center,
    fill=headblue!7, text width=0.90\columnwidth, inner sep=3pt
  },
  outcome/.style={
    draw=done!80!black, rounded corners=2pt, align=center,
    fill=done!10, text width=0.90\columnwidth, inner sep=3pt
  },
  arrow/.style={-{Latex[length=2mm]}, thick, draw=headblue!80}
]
\node[stage] (s0) {\textbf{Whole-report logic}\\
Modeling $\rightarrow$ controller design $\rightarrow$ optimization $\rightarrow$ validation $\rightarrow$ submission package};
\node[stage, below=of s0] (s1) {\textbf{Sections 1--2}\\
System model, equations of motion, and SMC design choices};
\node[stage, below=of s1] (s2) {\textbf{Sections 3--4}\\
PSO methodology, robust tuning setup, and Lyapunov stability proofs};
\node[stage, below=of s2] (s3) {\textbf{Sections 5--7}\\
Monte Carlo benchmarks, chattering/energy analysis, boundary layer and robustness studies};
\node[stage, below=of s3] (s4) {\textbf{Section 8 + Appendix B}\\
Execution timeline, closeout checklist, and next-iteration development priorities};
\node[stage, below=of s4] (s5) {\textbf{Appendices A/C/D}\\
Gain reference, literature positioning snapshot, and notation glossary};
\node[outcome, below=of s5] (out) {\textbf{Output}\\
submission-ready technical package with traceable evidence and clear next-cycle roadmap};
\draw[arrow] (s0) -- (s1);
\draw[arrow] (s1) -- (s2);
\draw[arrow] (s2) -- (s3);
\draw[arrow] (s3) -- (s4);
...
\paragraph{Simplification summary (Section 1.4, PSO only).}
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  \item \textbf{Baseline kept:} same state and actuation structure as
  \eqref{eq:eom} and \eqref{eq:state_space}, with unchanged bounds
  $u\in[-150,150]$~N and $\Delta t=0.01$~s.
  \item \textbf{Linearization step:} numerical $(\mathbf{A},\mathbf{B})$
  around upright equilibrium $(\vq^\star,\dot{\vq}^\star)=(\mathbf{0},\mathbf{0})$
  using $\varepsilon=10^{-8}$.
  \item \textbf{Term reduction:} coupling terms scaled down (0.7--0.8 range)
  and gyroscopic augmentation disabled during PSO loops.
  \item \textbf{Scope guard:} all final figures/tables use the full nonlinear model;
  simplification is used only to reduce PSO search runtime (about 10--50$\times$).
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \begin{equation}
    \sigma = \lambda_1(\theta_1-\theta_1^\star)
    + \lambda_2(\theta_2-\theta_2^\star)
    + k_1(\dot{\theta}_1-\dot{\theta}_1^\star)
    + k_2(\dot{\theta}_2-\dot{\theta}_2^\star)
    \label{eq:surface}
  \end{equation}
  ...
  \begin{equation}
    \begin{aligned}
      \dot{\sigma} &=
      \lambda_1\dot{\theta}_1 + \lambda_2\dot{\theta}_2
      + \mathbf{L}\mathbf{M}^{-1}\!\left(\mathbf{B}u-\mathbf{F}\right)
      + \mathbf{L}\mathbf{M}^{-1}\mathbf{B}\,d(t), \\
      \mathbf{L} &= [0,\;k_1,\;k_2], \qquad
      \mathbf{F} = \mathbf{C}\dot{\vq}+\mathbf{G}+\mathbf{F}_\text{fric}.
    \end{aligned}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \begin{align}
    ...
    u_2   &= -K_2\!\int_0^t \sign(\sigma(\tau))\,d\tau
    \qquad \text{(integral memory)} \notag
  \end{align}
...
\caption{PSO convergence evidence from MT-6 logs: best-fitness trend, swarm-mean fitness, and parameter evolution.
The displayed run terminated at iteration 30 via early-stop; configured maximum remains $T=200$.}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Hyperparameter sensitivity note:}
  ...
  \item \textbf{Cost scale calibration note:}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Scenario split rationale note:}
...
\caption{MT-7 PSO reproducibility: chattering index (mean $\pm$ SD) for seeds 42--51
  under challenging initial conditions.
  Per-seed sample counts vary after filtering ($n=3$--$7$, total $n=49$);
  global mean $=107.6$ with $\text{CV}=5.1\%$.}
\label{fig:pso_seeds}
\end{figure}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Known consistency note:}
...
\begin{itemize}[noitemsep]
  ...
  \texttt{mt8\_repro\_seed42\_*.json} experiment files; seed protocol verified).
\end{itemize}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Observed success:} 3/4 controllers produced valid stabilization traces;
        Hybrid Adaptive returned sentinel failure outputs in all 100 MT-5 runs.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{experiments/figures/LT7_section_7_2_transient_response.png}
\caption{Transient-response summary (MT-5 Monte Carlo): settling time and overshoot
  with uncertainty bars for the four controllers.
  This quantitative summary is used for controller ranking, rather than single-run traces.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{experiments/figures/LT7_section_7_3_chattering.png}
\caption{Frequency-domain chattering summary (QW-2):
  chattering index and high-frequency control-energy share ($>10$~Hz) across controllers.
  This is the official actuator-wear metric used for chattering comparisons.}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Metric disambiguation note:}
...
\paragraph{Official metric decision:}
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
...
STA vs.\ Hybrid       & --   & --    & --   & N/A (Hybrid sentinel failure in MT-5) \\
Hybrid vs.\ Adaptive  & --   & --    & --   & N/A (Hybrid sentinel failure in MT-5) \\[0.3em]
...
\caption{Radar chart summarising all four controllers across five normalised metrics
  (higher = better in each dimension), shown for qualitative overview only.
  Quantitative ranking in this report uses the MT-5/QW-2 tables; Hybrid metrics are not
  used for MT-5 statistical comparisons due to sentinel failures.}
\label{fig:radar}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{experiments/figures/LT7_section_7_4_energy.png}
\caption{Control-energy summary from QW-2:
  total control energy and peak power across controllers.
  The panel supports the energy trade-off discussion in this section.}
...
\caption{MT-6 audit view using the legacy time-domain chattering index and $\theta_1$
  overshoot for fixed ($\varepsilon=0.02$) versus adaptive
  ($\varepsilon_{\min}=0.0025$, $\alpha=1.21$) boundary layer.
  Retained for traceability; final claims use the corrected FFT metric below.}
...
\caption{Corrected MT-6 chattering metric (FFT RMS in the high-frequency band):
  fixed baseline versus adaptive set B, 100 runs (seed 42).
  Observed reduction is 3.7\% ($1.9959\times10^{-4}\rightarrow1.9230\times10^{-4}$),
  indicating only marginal benefit from adaptive boundary layering.}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Study limitations note:}
  only three $\varepsilon$ values tested; no fine sweep.
  \item Study used full nonlinear model \eqref{eq:eom}.
  \item Corrected metric implementation for MT-6 used FFT RMS above 20~Hz;
  QW-2 ranking uses the report-wide high-frequency definition ($>10$~Hz).
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \textbf{Measurement protocol note:}
...
\subsection{Disturbance Stress-Test (MT-8)}

\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  \item Disturbance test: external cart disturbances (step, impulse, sinusoidal, random).
  \item Logged attenuation metric (for audit): $1 - \|\vx_\text{dist}\|/\|\vx_\text{nom}\|$.
  \item Data-validity note: all controller-scenario entries are marked
  \texttt{converged=false} in the MT-8 disturbance audit log;
  therefore attenuation ratios are not used for quantitative ranking.
\end{itemize}
...
\toprule
\textbf{Controller} & \textbf{Converged scenario entries} & \\
\midrule
STA-SMC               & 0/4 & all disturbance summaries non-converged \\
Hybrid Adaptive STA   & 0/4 & all disturbance summaries non-converged \\
Classical SMC         & 0/4 & all disturbance summaries non-converged \\
Adaptive SMC          & 0/4 & all disturbance summaries non-converged \\
\bottomrule
\end{tabular}%
}
\end{center}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  \item This subsection is used to document stress behaviour and control-effort
  trends only; disturbance-rejection ranking is deferred pending converged reruns.
\end{itemize}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{experiments/figures/fig06_disturbance_effort.png}
\caption{Control effort increase (\%) under four disturbance types (MT-8 stress-test logs).
  ...
  (136\%).  These values are used for qualitative effort-trend discussion only,
  because corresponding raw runs are marked non-converged.}
...
\subsection{Study Map: Why and Result of Each Study}

\begin{table*}[t]
\centering
\footnotesize
\begin{tabularx}{\textwidth}{@{}p{1.2cm}p{5.2cm}X@{}}
\toprule
\textbf{Study} & \textbf{Why} & \textbf{Main result} \\
\midrule
MT-5 &
Benchmark 4 controllers under realistic near-upright random ICs (100 MC runs/controller). &
STA-SMC achieved best settling/overshoot (1.82~s, 2.3\%); Hybrid Adaptive returned sentinel failure values (energy $=10^6$, chattering $=0$). \\
MT-6 &
Quantify boundary-layer $\varepsilon$ trade-off and validate adaptive-boundary claim. &
Corrected FFT metric shows only 3.7\% chattering reduction; fixed $\varepsilon=0.02$ is near-optimal. \\
MT-7 &
Stress-test PSO gain generalization under harder initial conditions. &
Single-scenario tuning degraded by 50.4$\times$ in chattering index; motivated robust multi-scenario PSO. \\
MT-8 &
Evaluate disturbance rejection and robust tuning performance on constrained scenarios. &
MT-8 stress logs were non-converged for all controller-scenario entries; results are retained only for qualitative effort trends and to flag unresolved robustness issues. \\
LT-6 &
Test parametric robustness under simultaneous model mismatch ($\pm10\%$, $\pm20\%$). &
All controllers remained stable at tested levels; mismatch tolerance ranking: Hybrid 16\% (best), Adaptive 15\%, Classical 12\%, STA 8\%. \\
LT-7 &
Package full evidence set for submission-quality paper deliverable. &
Paper status: submission-ready (v2.1), 71 pages, 14 figures, and 1,300+ simulations documented. \\
QW-2 &
Resolve conflicting chattering interpretations using actuator-wear-relevant frequency analysis. &
Official metric set to FFT high-frequency band ($>10$~Hz); by this metric STA-SMC is 74\% better than Classical. \\
\bottomrule
\end{tabularx}
\end{table*}
...
\subsection{Execution Plan to Submission (March--April 2026)}
\label{sec:timeline_2026}

\noindent\textbf{Objective:} convert the current 90/100 draft into a
submission-ready thesis package through four explicit closure gates.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  All 9 figures meet 300~DPI; file sizes are below 600~KB
  (largest: MT-6 PSO convergence, about 570~KB).
\end{itemize}

\subsection{Closeout Priorities Before Submission}
\begin{center}
\small
\begin{tabularx}{\columnwidth}{@{}p{2.1cm}Xp{2.2cm}@{}}
\toprule
\textbf{Priority} & \textbf{Action required} & \textbf{Target date} \\
\midrule
P0 (blocker) & Finalize STA gain-order consistency package: constrained rerun with $K_1>K_2$ or formal derivation that justifies the reported ordering. & Before advisor sign-off \\
P1 (methods) & Complete the Appendix~\ref{app:issues} checklist items for methods clarity and reproducibility packaging. & Mar 28, 2026 \\
P1 (evidence) & Freeze final figure/table set and run final consistency QA across claims, captions, and logs. & Mar 14, 2026 \\
P2 (package) & Final bibliography/proofreading pass and advisor review integration. & Apr 15, 2026 \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{Reader Guide (Appendices A--D)}
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  \item Appendix~\ref{app:gains}: controller gains lookup table for implementation and reproducibility.
  \item Appendix~\ref{app:issues}: submission checklist and packaging action queue.
  \item Appendix~\ref{app:litcomp}: compact literature-positioning summary.
  \item Appendix~\ref{app:glossary}: symbol and notation lookup for quick review.
\end{itemize}

% ============================================================
\appendix
\section{Operational Gain Reference (Lookup Only)}
\label{app:gains}
% ============================================================
...
% ============================================================
\section{Appendix B: Submission Checklist and Packaging Plan}
\label{app:issues}
% ============================================================

\noindent This appendix is a closeout checklist for submission packaging.
\textbf{Appendix letter B} denotes this closeout package and development queue.
Detailed internal debugging logs are maintained outside the report body.

\begin{center}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lccp{5.9cm}@{}}
\toprule
\textbf{Track} & \textbf{Items} & \textbf{Open} & \textbf{Completion rule} \\
\midrule
B.1 Mandatory closeout & 4 & 1 & Must be completed before advisor sign-off \\
B.2 Advisor-ready reinforcement & 5 & 0 & Complete before final review meeting \\
B.3 Important development priorities & 6 & 6 & Execute in the next development cycle (non-blocking for submission) \\
B.4 Scope and exclusions & 3 & 0 & Keep comparative claims within declared scope \\
B.5 Completed actions log & 4 & 0 & Recorded for traceability \\
\midrule
\textbf{Total} & \textbf{22} & \textbf{7} & \\
\bottomrule
\end{tabular}%
}
\end{center}

\subsection{Mandatory Closeout Before Submission}
\begin{itemize}[leftmargin=1.5em,itemsep=0.25em]
  \item \textbf{STA consistency package (open):} finalize either constrained
  robust rerun enforcing $K_1 > K_2$ or a documented derivation that justifies
  the reported ordering for this DIP formulation.
  \item \textbf{Figure/table lock:} freeze the audited figure set now used in
  Sections~5--7, with caption-to-table consistency check.
  \item \textbf{Metric lock:} keep FFT high-frequency chattering metric as the
  official ranking metric for final comparative claims.
  \item \textbf{Scope lock:} keep MT-8 disturbance plots explicitly labeled as
  qualitative stress-test evidence only.
\end{itemize}

\subsection{Advisor-Ready Reinforcement Package}
\begin{itemize}[leftmargin=1.5em,itemsep=0.25em]
  \item Add compact derivation notes for the full model objects
  ($\mathbf{M}, \mathbf{C}, \mathbf{G}$) and key control terms.
  \item Add explicit Monte Carlo protocol notes (IC distribution, failure rule,
  and seed protocol).
  \item Add a short statistical-method note (assumption checks and
  effect-size definition).
  \item Add a one-paragraph definition block for robustness mismatch terminology.
  \item Keep the notation glossary synchronized with symbols used in
  Sections~2--7.
\end{itemize}

\subsection{Important Development Priorities (Next Iteration)}
\noindent\textbf{Detailed items:}
\begin{itemize}[leftmargin=1.5em,itemsep=0.25em]
  \item \textbf{Hybrid controller redevelopment:} redesign adaptation/rate-limiter
  interaction and retune for stable near-upright operation before re-entry into
  quantitative ranking.
  \item \textbf{MT-8 disturbance package upgrade:} add convergence-qualified
  disturbance benchmarks so disturbance claims can move from qualitative to
  quantitative.
  \item \textbf{Constrained robust PSO automation:} enforce controller-specific
  gain constraints directly in the optimizer pipeline with constraint-violation
  logging and rerun guards.
  \item \textbf{Automated report consistency QA:} add a pre-release checker for
  caption/table/log alignment and metric-name consistency across sections.
  \item \textbf{MPC batch integration:} integrate solver workflow into the
  experiment runner so MPC can be included in the same reproducibility path.
  \item \textbf{Derivation artifact export:} generate compact symbolic model
  artifacts ($\mathbf{M},\mathbf{C},\mathbf{G}$ and key control terms) directly
  from source scripts for appendix sync.
\end{itemize}

\subsection{Scope and Exclusions Used for Final Claims}
\begin{center}
\small
\begin{tabularx}{\columnwidth}{@{}p{2.2cm}Xp{2.0cm}@{}}
\toprule
\textbf{Controller} & \textbf{Reporting scope in this report} & \textbf{Claim status} \\
\midrule
Classical SMC & Included in all comparative benchmarks. & Included \\
STA-SMC & Included in all comparative benchmarks and lead claims. & Included \\
Adaptive SMC & Included in all comparative benchmarks. & Included \\
Hybrid Adaptive STA & Retained for architecture completeness; excluded from MT-5 statistical ranking due to non-operational run outputs in current logs. & Excluded \\
Swing-Up & Not part of the near-upright stabilization benchmark objective. & Out of scope \\
MPC & Not integrated in the batch-runner workflow for this report cycle. & Out of scope \\
\bottomrule
\end{tabularx}
\end{center}

\subsection{Completed Actions (Traceability)}
\begin{itemize}[leftmargin=1.5em,itemsep=0.25em]
  \item Figure references in Sections~5--7 were aligned with numerically
  consistent assets.
  \item Contradictory Hybrid ranking claims were removed from MT-5 statistical
  comparisons.
  \item Boundary-layer discussion was aligned to the corrected
  frequency-domain metric.
  \item Repository metadata/link line was removed from the report title block.
\end{itemize}

% ============================================================
\section{Literature Positioning Snapshot}
\label{app:litcomp}
% ============================================================
...
\textbf{This work (STA-SMC)} & \textbf{1.82} & \textbf{74\%} &
  \textbf{LT-6: stable at tested $\pm20\%$} &
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item Unlike all three prior works, this project reports simultaneous Lyapunov
  proofs, explicit non-convergence flags for disturbance stress logs, and 400+ Monte
  Carlo runs across controllers.
