...
class TestCreateControllerTypeValidation:
    ...
    def test_numeric_controller_type_raises_error(self, mock_config):
        """Should raise error for numeric controller type."""
...
class TestCreateControllerGainsValidation:
    ...
    def test_sta_smc_requires_k1_greater_than_k2(self, mock_config):
        """Should reject STA gains that violate K1 > K2 stability constraint."""
        with pytest.raises((ValueError, ConfigValueError), match="K1 > K2"):
            create_controller(
                'sta_smc',
                config=mock_config,
                gains=[2.0, 6.0, 5.0, 3.0, 4.0, 2.0],  # K1 <= K2 (invalid)
            )
    ...
    def test_zero_gains_raise_error(self, mock_config):
        """Should reject zero gains because factory gain validation requires positivity."""
        with pytest.raises((ValueError, ConfigValueError)):
            create_controller(
                'classical_smc',
                config=mock_config,
...
class TestCreateControllerConfiguration:
    ...
    def test_explicit_sta_gains_not_overridden_by_config(self):
        """Explicit gains must remain source of truth even when config has different STA gains."""
        class SimpleConfig:
            def __init__(self):
                self.controllers = {
                    "sta_smc": {
                        "gains": [8.0, 4.0, 12.0, 6.0, 4.85, 3.43],  # conflicting config gains
                        "max_force": 150.0,
                        "dt": 0.001,
                        "boundary_layer": 0.3,
                    }
        ...
        explicit_gains = [9.0, 3.0, 7.0, 2.0, 4.0, 1.5]
        controller = create_controller("sta_smc", config=SimpleConfig(), gains=explicit_gains)

        assert controller is not None
        assert hasattr(controller, "gains")
        np.testing.assert_allclose(controller.gains, explicit_gains)
...
class TestCreateControllerEdgeCases:
    ...
    def test_very_large_gains_raise_error(self, mock_config):
        """Should reject very large gain values to avoid numerical instability."""
        large_gains = [1e6, 1e6, 1e6, 1e6, 1e6, 1e6]
        with pytest.raises((ValueError, ConfigValueError), match="too large"):
            create_controller('classical_smc', config=mock_config, gains=large_gains)
