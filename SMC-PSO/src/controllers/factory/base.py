...
def create_controller(controller_type: str,
    ...
    if (
        ...
        and isinstance(controller_gains, (list, tuple, np.ndarray))
        and len(controller_gains) > 4
    ):
        # Backward compatibility: legacy hybrid pipelines sometimes pass
        # 6-element gain vectors. Hybrid surface design uses the first 4 only.
        logger.warning(
            "Hybrid controller received %d gains; using first 4 surface gains [c1, lambda1, c2, lambda2].",
            len(controller_gains),
        )
        controller_gains = list(controller_gains[:4])
    if (
        controller_type == 'adaptive_smc'
        and isinstance(controller_gains, (list, tuple, np.ndarray))
        and len(controller_gains) > 5
    ):
        # Backward compatibility: some legacy robustness scripts emit
        # 6-element vectors; adaptive controller uses first 5 gains.
        logger.warning(
            "Adaptive controller received %d gains; using first 5 gains [k1, k2, lambda1, lambda2, gamma].",
            len(controller_gains),
        )
        controller_gains = list(controller_gains[:5])

    # Validate gains with controller-specific rules.
    # validate_controller_gains returns ValidationResult (does not raise), so
    # we must explicitly reject invalid results here.
    gain_validation = validate_controller_gains(controller_gains, controller_type)
    if not gain_validation.valid:
        ...
        if not gain_validation.valid:
            error_detail = "; ".join(gain_validation.errors) or gain_validation.get_summary()
            raise ValueError(f"Invalid gains for '{controller_type}': {error_detail}")
    ...
    # Never allow config-side gains to override selected/validated gains.
    controller_params.pop("gains", None)
    ...
    def _resolve_positive_param(name: str, fallback: float) -> float:
        """Resolve numeric config parameter from controller/global config with safe fallback."""
        raw_value = controller_params.get(name)
        if raw_value is None and config is not None:
            if isinstance(config, dict):
                raw_value = config.get(name)
            else:
                raw_value = getattr(config, name, None)
    ...
    def _resolve_hybrid_mode(raw_mode: Any) -> HybridMode:
        """Resolve hybrid mode from enum/string with robust fallback."""
        if isinstance(raw_mode, HybridMode):
            return raw_mode
        if isinstance(raw_mode, str):
            ...
            for mode in HybridMode:
                if mode.value == normalized or mode.name.lower() == normalized:
                    return mode
        return HybridMode.CLASSICAL_ADAPTIVE

    # Shared defaults used across controller configuration and fallback paths.
    control_dt = _resolve_positive_param("dt", 0.01)
    control_max_force = _resolve_positive_param("max_force", 150.0)
    hybrid_mode = _resolve_hybrid_mode(controller_params.get("hybrid_mode"))
    ...
    try:
        ...
        if controller_type == 'hybrid_adaptive_sta_smc':
            ...
            # Prevent conflicting top-level overrides for managed hybrid fields.
            hybrid_extra_params = dict(controller_params)
            for reserved in (
                'gains',
                'dt',
                'max_force',
            ...
            config_params = {
                ...
                'max_force': control_max_force,  # Required by all controllers
                'dt': control_dt,  # Safe default for all
                **controller_params
            }
            ...
            # Preserve the resolved/validated gains as final source of truth.
            if controller_type != 'mpc_controller':
                config_params['gains'] = controller_gains
    ...
    try:
        ...
        if controller_type == 'hybrid_adaptive_sta_smc':
            ...
            if controller_type == 'mpc_controller':
                ...
                fallback_params = {
                    'gains': controller_gains,
                    'max_force': control_max_force,  # Always required
                    'dt': control_dt  # Always add dt for safety
                }
