...
class StateSpaceUtilities:
    ...
    def compute_energy(state: np.ndarray, mass_matrix: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        -------
        np.ndarray
            KINETIC energy only. Gravitational/potential energy is NOT added here
            (it is system-specific; see plant models). The original docstring said
            "total energy" but the implementation returns kinetic energy only.

        Notes
        -----
        Assumes the CANONICAL GROUPED convention
        ``[x, theta1, theta2, x_dot, theta1_dot, theta2_dot]`` so that the second
        half of the vector is the velocity block. Passing an INTERLEAVED state
        would silently mix positions and velocities (Trap A). See
        ``.agents/brain/STATE_VECTOR_CONVENTION.md``.
        """
        # Assume state is [positions, velocities] (grouped convention)
