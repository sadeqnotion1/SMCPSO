...
# Value: 10^-12 ~ 1000x machine epsilon for double precision
...
def safe_divide(
    ...
    Mathematical Definition:
        safe_divide(a, b) = a / max(|b|, epsilon) * sign(b)
    ...
    Notes:
        - For denominators with |b| < epsilon, uses sign-preserving replacement
    ...
    # Preserve sign: safe_divide(a, b) = a / (epsilon * sign(b)) if |b| < epsilon
...
def safe_reciprocal(
    ...
    Mathematical Definition:
        safe_reciprocal(x) = 1 / max(|x|, epsilon) * sign(x)
...
def safe_sqrt(
    ...
    Clips input to [min_value, inf) before applying sqrt to prevent domain errors
    from numerical noise producing negative values.

    Mathematical Definition:
        safe_sqrt(x) = sqrt(max(x, min_value))
    ...
    Notes:
        - Designed for control systems where sqrt(x^2) may produce negative x
...
def safe_log(
    ...
    Clips input to [min_value, inf) before applying log to prevent domain errors.
    ...
    Notes:
        ...
        - For log(0) -> -inf behavior, use min_value ~ 1e-300 (near underflow)
    """
    if min_value <= 0:
...
def safe_exp(
    ...
    Clips input to (-inf, max_value] before applying exp to prevent overflow.
...
def safe_norm(
    ...
    Args:
        ...
        ord: Norm order (default: 2 for Euclidean norm)
             ...
             - 2: L2 norm (Euclidean)
             - np.inf: L-inf norm (maximum absolute value)
