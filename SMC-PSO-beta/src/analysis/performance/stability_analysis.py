...
class _FallbackRegularizer:
    """Minimal AdaptiveRegularizer stand-in when src.plant.core is unavailable.

    Adds Tikhonov ridge regularization scaled to conditioning so the SVD-based
    Lyapunov fallback still functions without the plant.core dependency.
    """

    def __init__(self, regularization_alpha=1e-6, max_condition_number=1e12,
                 min_regularization=1e-12, use_fixed_regularization=False):
        self.regularization_alpha = regularization_alpha
        self.max_condition_number = max_condition_number
        self.min_regularization = min_regularization
        self.use_fixed_regularization = use_fixed_regularization

    def regularize_matrix(self, M):
        ...
        try:
            ...
            cond = np.inf
        if not np.isfinite(cond) or cond > self.max_condition_number:
            ridge = max(self.regularization_alpha, self.min_regularization)
            return M + ridge * np.eye(n)
        return M
...
class StabilityAnalyzer(PerformanceAnalyzer):
    ...
    def _compute_stability_margins(self, frequencies: np.ndarray, H: np.ndarray) -> Dict[str, Any]:
        ...
        # Gain margin (at phase crossover frequency where phase = -180 deg)
        phase_crossover_indices = np.where(
            np.abs(phase_deg + 180.0) < 5.0  # Within 5 degrees of -180 deg
        )[0]
    ...
    def _estimate_growth_rate(self, signal: np.ndarray) -> float:
        ...
        # Fit exponential growth: y = A * exp(lambdat)
        try:
    ...
    def _analyze_analytical_lyapunov(self, A: np.ndarray) -> Dict[str, Any]:
        ...
        """
        try:
            # M8-SCHED-3: guard the optional plant.core dependency.
            try:
                from src.plant.core.numerical_stability import (
                    AdaptiveRegularizer
                )
            except ImportError:
                AdaptiveRegularizer = _FallbackRegularizer
    ...
    def _solve_lyapunov_svd(
        ...
        # Vectorize Lyapunov equation: (I (x) A^T + A^T (x) I) vec(P) = -vec(Q)
