...
class _SimulatedData:
    """Lightweight DataProtocol implementation for re-simulated trajectories.

    Produced by RobustnessAnalyzer's linear state-space re-simulation so that
    perturbed / initial-condition / disturbance analyses operate on genuinely
    re-simulated states rather than the nominal data returned unchanged.
    """

    def __init__(self, times: np.ndarray, states: np.ndarray, controls: np.ndarray) -> None:
...
class RobustnessAnalyzer(PerformanceAnalyzer):
        ...
        **kwargs
        ...
        try:
            ...
            if uncertainty_models:
                kw = kwargs.copy()
                kw.pop('uncertainty_models', None)
                structured_analysis = self._analyze_structured_uncertainties(data, uncertainty_models, **kw)
    ...
    def _perform_sensitivity_analysis(self, data: DataProtocol, **kwargs) -> Dict[str, Any]:
        ...
        if parameter_ranges:
            kw = kwargs.copy()
            kw.pop('parameter_ranges', None)
            param_sensitivity = self._analyze_parameter_sensitivity(data, parameter_ranges, **kw)
    ...
    def _analyze_disturbance_sensitivity(self, data: DataProtocol, **kwargs) -> Dict[str, Any]:
        ...
        for level in disturbance_levels:
            ...
            disturbed_data = self._apply_disturbance_to_data(data, disturbance, **kwargs)
            if disturbed_data:
    ...
    def _perform_worst_case_analysis(self, data: DataProtocol, **kwargs) -> Dict[str, Any]:
        ...
        kw = kwargs.copy()
        kw.pop('parameter_ranges', None)
        worst_case_results = self._grid_search_worst_case(data, parameter_ranges, performance_func, grid_points, **kw)

        # Vertex analysis (check parameter combination vertices)
        vertex_results = self._vertex_analysis(data, parameter_ranges, performance_func, **kw)
    ...
    def _linear_state_space_resim(self, matrices: Tuple[np.ndarray, ...], data: DataProtocol,
                                  x0: Optional[np.ndarray] = None,
                                  extra_input: Optional[np.ndarray] = None) -> Optional[DataProtocol]:
        """Re-simulate the linear state-space model x_dot = A x + B u.

        Uses the provided (A, B, C, D) matrices together with the input signal,
        time grid, and initial state already present in ``data``. This keeps the
        analysis decoupled from the nonlinear plant/controller packages: it is a
        genuine re-simulation of the *linearized* model (an approximation of the
        underlying nonlinear system), not the nominal data returned unchanged.

        Returns None if re-simulation cannot be performed (missing matrices or
        inputs, or a shape mismatch), so callers fall back honestly.
        """
        try:
            A, B, C, D = matrices[0], matrices[1], matrices[2], matrices[3]
            if not hasattr(data, 'times') or not hasattr(data, 'controls'):
                return None
            t = np.asarray(data.times, dtype=float)
            u = np.asarray(data.controls, dtype=float)
            if t.ndim != 1 or len(t) < 2:
                return None

            # Align the input signal so samples run along axis 0 (time).
            if u.ndim == 1:
            ...
            if extra_input is not None:
                u = u + np.asarray(extra_input, dtype=float).reshape(u.shape)

            if x0 is None:
                states0 = np.asarray(data.states, dtype=float)
                x0_vec = states0[0] if states0.ndim > 1 else np.atleast_1d(states0[0])
            else:
                x0_vec = np.atleast_1d(np.asarray(x0, dtype=float))

            _, _, xout = signal.lsim((A, B, C, D), U=u, T=t, X0=x0_vec)
            xout = np.atleast_2d(xout)
            if xout.shape[0] != t.shape[0]:
                xout = xout.T
            return _SimulatedData(times=t, states=xout, controls=u)
        except Exception:
            return None

    def _simulate_perturbed_system(self, data: DataProtocol, perturbed_matrices: Tuple[np.ndarray, ...], **kwargs) -> Optional[DataProtocol]:
        """Simulate the system with perturbed parameters via linear re-simulation.

        Integrates the perturbed (A, B, C, D) state-space model using the nominal
        input/time/initial-state from ``data``. Returns a re-simulated
        DataProtocol, or None if re-simulation is not possible.
        """
        if perturbed_matrices is None or len(perturbed_matrices) < 4:
            return None
        return self._linear_state_space_resim(perturbed_matrices, data)

    def _simulate_with_initial_conditions(self, data: DataProtocol, initial_conditions: np.ndarray, **kwargs) -> Optional[DataProtocol]:
        """Re-simulate the linear model from perturbed initial conditions.

        Requires ``system_matrices`` in kwargs (the (A, B, C, D) tuple). Returns a
        re-simulated DataProtocol, or None if matrices are unavailable.
        """
        system_matrices = kwargs.get('system_matrices')
        if system_matrices is None or len(system_matrices) < 4:
            return None
        return self._linear_state_space_resim(system_matrices, data, x0=initial_conditions)

    def _apply_disturbance_to_data(self, data: DataProtocol, disturbance: np.ndarray, **kwargs) -> Optional[DataProtocol]:
        """Re-simulate the linear model with an additive input disturbance.

        The disturbance is added to the input signal and the linear model is
        re-simulated. Requires ``system_matrices`` in kwargs; returns None if
        matrices are unavailable.
        """
        system_matrices = kwargs.get('system_matrices')
        if system_matrices is None or len(system_matrices) < 4:
            return None
        return self._linear_state_space_resim(system_matrices, data, extra_input=disturbance)
