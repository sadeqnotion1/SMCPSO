...
equal to the number of state variables.  An
...
def linearize_dip(dyn: callable, x_eq: np.ndarray, u_eq: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    # M8-SCHED-2: lazy-guard the optional MPC dependency (decoupled import).
    try:
        from src.controllers.mpc.mpc_controller import _numeric_linearize_continuous
    except ImportError as exc:  # pragma: no cover - optional dependency
        raise ImportError(
            "linearize_dip requires src.controllers.mpc.mpc_controller."
            "_numeric_linearize_continuous, which is unavailable in this environment."
        ) from exc
...
def controllability_matrix(A: np.ndarray, B: np.ndarray) -> np.ndarray:
    ...
    The system is controllable if this matrix has full row rank equal to ``n``.
...
def observability_matrix(A: np.ndarray, C: np.ndarray) -> np.ndarray:
    ...
    when this matrix has full column rank equal to ``n``.
...
def check_controllability_observability(A: np.ndarray, B: np.ndarray, C: np.ndarray) -> Tuple[bool, bool]:
    ...
    (bool, bool)
        ...
        corresponding rank test passes.
    """
    n = A.shape[0]
