...
class ParameterEstimationGenerator(ResidualGenerator):
    ...
    def _estimate_parameters(self, states: np.ndarray, controls: np.ndarray, times: np.ndarray) -> Dict[str, float]:
        """Estimate parameters from a data window.

        Not implemented. A faithful port must not fabricate estimates: the original
        source returned nominal parameter values perturbed by random noise (non-reproducible
        placeholder output) which is scientifically misleading if consumed as real
        system-identification output. Until a proper routine (for example recursive
        least squares) is implemented, this fails loudly rather than returning fake
        data. See AUDIT_LEDGER.md finding M8-S2-2.
        """
        raise NotImplementedError(
            "ParameterEstimationGenerator._estimate_parameters is not implemented. "
            "The original placeholder fabricated estimates via randomised noise around nominal values; a real "
            "system-identification routine (for example recursive least squares) is required."
        )
