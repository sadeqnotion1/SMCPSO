...
class PhysicsConfig(StrictModel):
    """
    Configuration model for the double-inverted pendulum physical plant parameters.

    These nominal values correspond to a standard double-inverted pendulum physical
    prototype used in classical control system benchmarks.
...
class ClassicalSMCConfig(_BaseControllerConfig):
    ...
    Uses a proportional sliding surface sliding design. Boundary layer techniques
    are configured here to replace the discontinuous signum control action, avoiding
    high-frequency control chattering.

    References
    ----------
    - Slotine, J.-J. E., & Li, W. (1991). Applied Nonlinear Control. Prentice Hall.
      (Boundary layer and sliding surface design principles).
    """
...
class STASMCConfig(_BaseControllerConfig):
    ...
    A second-order sliding mode controller that guarantees finite-time convergence
    while rejecting bounded disturbances and smooth chattering.

    References
    ----------
    - Moreno, J. A., & Osorio, M. (2008). "A Lyapunov approach to second-order
      sliding mode controllers and observers." 47th IEEE Conference on Decision
      and Control (CDC). doi:10.1109/CDC.2008.4739356
    """
    max_force: Optional[float] = Field(None, gt=0.0)
...
class AdaptiveSMCConfig(_BaseControllerConfig):
    ...
    Adaptively adjusts sliding gains online based on tracking error sliding proximity
    to compensate for unknown model uncertainties.

    References
    ----------
    - Slotine, J.-J. E., & Li, W. (1991). Applied Nonlinear Control. Prentice Hall.
      (For Lyapunov design of adaptation laws and dead-band handling).
    """
    max_force: Optional[float] = Field(None, gt=0.0)
...
class HybridAdaptiveSTASMCConfig(_BaseControllerConfig):
    ...
    Combines the online gain adaptation mechanism with second-order super-twisting
    sliding mode action. Regulates the constraint that the smooth saturation
    boundary layer thickness (`sat_soft_width`) must be at least as wide as the
    adaptive dead zone (`dead_zone`).

    References
    ----------
    - Slotine, J.-J. E., & Li, W. (1991). Applied Nonlinear Control. Prentice Hall.
      (Boundary layer and dead-band constraints).
    - Moreno, J. A., & Osorio, M. (2008). "A Lyapunov approach to second-order
      sliding mode controllers and observers." 47th IEEE Conference on Decision
      and Control (CDC). doi:10.1109/CDC.2008.4739356
      (Super-twisting convergence bounds).
    """
    max_force: Optional[float] = Field(None, gt=0.0)
    dt: Optional[float] = Field(None, gt=0.0)
