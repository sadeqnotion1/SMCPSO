...
class FullFidelityPhysicsComputer:
    ...
    def _compute_full_inertia_matrix_numba(
        ...
        # Full inertia matrix with all coupling terms (corrected)
        M11 = m0 + m1 + m2
        M12 = (m1 * Lc1 + m2 * L1) * c1
        M13 = m2 * Lc2 * c2
    ...
    def _compute_full_coriolis_matrix_numba(
        ...
        if include_coriolis and include_centrifugal:
            ...
            C[0, 1] = -a1 * s1 * theta1_dot
            C[0, 2] = -a2 * s2 * theta2_dot
            C[1, 2] = a12 * s12 * theta2_dot
            C[2, 1] = -a12 * s12 * theta1_dot

        # Note: Fabricated gyroscopic term removed to preserve passive physical energy structure
