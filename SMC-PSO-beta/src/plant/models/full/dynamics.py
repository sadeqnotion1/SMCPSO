...
class FullDIPDynamics(BaseDynamicsModel):
    ...
    def __init__(
        self,
        config: Union[FullDIPConfig, Dict[str, Any], Any],
        ...
        Args:
            config: Validated configuration for full DIP model or dictionary/AttributeDictionary
        ...
        if isinstance(config, dict):
            ...
            self.config = config
        elif type(config).__name__ == "AttributeDictionary" or hasattr(config, "to_dict"):
            # Convert AttributeDictionary/duck-typed config to dict and create FullDIPConfig
    ...
    def _rhs_core(self, state: np.ndarray, u: float = 0.0) -> np.ndarray:
        ...
        Args:
            state: System state vector [x, theta1, theta2, x_dot, theta1_dot, theta2_dot]
